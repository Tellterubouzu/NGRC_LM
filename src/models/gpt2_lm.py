...
def parse_args():
    ...
    # Weights & Biases
    parser.add_argument("--wandb_project", type=str, default="gpt2-scratch-pretrain-64M", help="WandB project name")
...
def main():
    ...
    if is_master:
        ...
        print(f"▶️ babylm テストセットで mean-so-far-PPL を最大 {args.seq_len * 8} トークンまで計測します…")
        test_ds = load_dataset(args.dataset_path, split="test", streaming=False)
        seq_len_test = args.seq_len * 4  # 2048
        buffer = []
        test_blocks = []
        for ex in test_ds:
        ...
        max_exp = int(math.log2(seq_len_test))
        report_ks = [4**n for n in range(max_exp + 1)]
