...
# =============================================================================
# Input Embeddings: fixed_sparse / learnable_lowdim
# =============================================================================
class FixedSparseEmbedding(nn.Module):
    """
    固定疎ランダム射影を密埋め込みの lookup と同じ O(B·d) のコストで実現する層。
    各トークン v に対して d 個の (reservoir_index, value) を事前サンプルしておき、
    forward では scatter_add で (B,N) を構成する。
    """
    def __init__(self, vocab_size:int, reservoir_size:int, d:int, sigma_in:float, device):
        super().__init__()
        self.vocab_size = vocab_size
        self.reservoir_size = reservoir_size
        self.d = d
        # 形：(V, d)
        idx = torch.empty(vocab_size, d, dtype=torch.long, device=device)
        val = torch.empty(vocab_size, d, dtype=torch.float32, device=device)
        # 各トークンでユニークな d 個のリザバー位置をサンプル
        for v in range(vocab_size):
            # ユニーク確保のため randperm を使用（コストは V*d で線形）
            inds = torch.randperm(reservoir_size, device=device)[:d]
            vals = torch.randn(d, device=device) * sigma_in
            idx[v] = inds
            val[v] = vals
        # checkpoint サイズを抑えるため persistent=False
        self.register_buffer("idx", idx, persistent=False)
        self.register_buffer("val", val.to(torch.bfloat16), persistent=False)

    @torch.no_grad()
    def forward(self, token_ids: torch.LongTensor) -> torch.Tensor:
        # token_ids: (B,)
        B = token_ids.size(0)
        idx = self.idx[token_ids]  # (B, d)
        val = self.val[token_ids]  # (B, d)
        out = torch.zeros(B, self.reservoir_size, device=token_ids.device, dtype=val.dtype)
        out.scatter_add_(1, idx, val)
        return out

class LearnableLowDimEmbedding(nn.Module):
    """
    学習可能な低次元埋め込み E: (V, emb_dim) と、N 次元への射影 P: (emb_dim, N)。
    P は固定/学習を選択可能（proj_trainable）。
    """
    def __init__(self, vocab_size:int, emb_dim:int, reservoir_size:int,
                 sigma_in:float, proj_trainable:bool, device):
        super().__init__()
        self.emb = nn.Embedding(vocab_size, emb_dim)  # 学習可
        # He/Xavier の簡易スケールで初期化
        nn.init.normal_(self.emb.weight, mean=0.0, std=sigma_in / math.sqrt(emb_dim))
        P = torch.randn(emb_dim, reservoir_size, device=device) * (sigma_in / math.sqrt(emb_dim))
        self.P = nn.Parameter(P, requires_grad=proj_trainable)

    def forward(self, token_ids: torch.LongTensor) -> torch.Tensor:
        e = self.emb(token_ids).float()      # (B, emb_dim)
        u = e @ self.P                       # (B, N)
...
class ESN_mlr(nn.Module):
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        r_out: int = 512,
        embed_mode: str = "fixed_sparse",   # "fixed_sparse" | "learnable_lowdim"
        emb_dim: int = 1024,                # learnable_lowdim 用
        ...
        # ---------- input embedding (switchable) ----------
        if embed_mode == "fixed_sparse":
            self.input_embed = FixedSparseEmbedding(vocab_size, reservoir_size, d, sigma_in, device)
        elif embed_mode == "learnable_lowdim":
            self.input_embed = LearnableLowDimEmbedding(vocab_size, emb_dim, reservoir_size, sigma_in, proj_trainable, device)
        else:
            raise ValueError("embed_mode must be fixed_sparse or learnable_lowdim")
    ...
    # ---------------- step() for fast decoding -----------------
    @torch.no_grad()
    def step(self, h: torch.Tensor, x_t: torch.LongTensor):
        """1 token step: carry-over hidden state for fast autoregressive decoding.
        x_t: (B,), h: (B,N)
        returns: (h_new, logits) where logits: (B,V)
        """
        u_proj = self.input_embed(x_t)                   # (B, N)
        with torch.amp.autocast(device_type="cuda", enabled=False):
            rec = torch.sparse.mm(self.W_rec.float(), h.float().t()).t()  # (B, N)
        u_proj = u_proj.to(h.dtype)
        rec    = rec.to(h.dtype)
        pre = (u_proj + rec).clamp_(-10.0, 10.0)
        a = self.a.unsqueeze(0)
        h_new = (1 - a) * h + a * self.act(pre)
        out = self.A(self.drop(self.B(h_new)))
        return h_new, out
    # ======================================================================
    # forward
    # ---------------- forward (training) -----------------
    def forward(self, x: torch.LongTensor, labels: torch.LongTensor | None = None):
        ...
        outs = []
        for t in range(T):
            u_proj = self.input_embed(x[:, t])
            with torch.amp.autocast(device_type="cuda", enabled=False):
                rec = torch.sparse.mm(self.W_rec.float(), h.float().t()).t()
            u_proj = u_proj.to(h.dtype)
            rec    = rec.to(h.dtype)
            pre = (u_proj + rec).clamp_(-10.0, 10.0)
            h = (1 - self.a) * h + self.a * self.act(pre)
            outs.append(self.A(self.drop(self.B(h))))
...
def sample_sequence(
    ...
    h = model.h0.expand(generated.size(0), -1).to(device)
    for _ in range(max_new_tokens):
        x_t = generated[:, -1]
        h, next_logits = model.step(h, x_t)
...
def parse_args_esn():
    ...
    parser.add_argument("--total_tokens", type=float, default=1e9, help="Total number of tokens to train on")
    ...
    # ESN
    parser.add_argument("--reservoir_size", type=int, default=9192, help="ESN reservoir size (N_rec)")
    ...
    # Embedding switch
    parser.add_argument("--embed_mode", choices=["fixed_sparse","learnable_lowdim"], default="fixed_sparse")
    parser.add_argument("--emb_dim", type=int, default=512)
    parser.add_argument("--proj_trainable", action="store_true")
    ...
    # Weights & Biases
    parser.add_argument("--wandb_project", type=str, default="ESN Lanugage Model_embed", help="WandB project name")
...
def ESN_experiment(lr):
    ...
    model = ESN_mlr(
        ...
        proj_trainable=args.proj_trainable,
        device=device,
    ...
    if not distributed or args.local_rank == 0:
            wandb_run_name = args.wandb_run_name or (
                f"ESN_mlr ({param_millions:.2f}M N{args.reservoir_size}_lr{args.learning_rate}"
                f"_bs{args.local_batch_size}_T{args.seq_len}_sigma{args.sigma_in}_rho{args.spectral_radius}"
                f"_spars{1-(args.d/args.reservoir_size):.6f}_drop{args.dropout}_rout{args.r_out}"
                f"_emb[{args.embed_mode}|{args.emb_dim}|Ptrain={args.proj_trainable}]_{id}"
            )
