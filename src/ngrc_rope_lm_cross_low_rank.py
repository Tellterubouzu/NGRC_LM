...
# =============================================================================
# RoPE (Rotary Positional Embedding) for embeddings
# =============================================================================
class RotaryEmbedding(nn.Module):
    """
    RoPE を (B, T, D) の「埋め込み」に直接適用する版。
    D は偶数である必要あり（奇数の場合は先頭の偶数次元だけに適用する）。
    """
    def __init__(self, dim: int, base: float = 10000.0):
        super().__init__()
        if dim <= 0:
            raise ValueError("RoPE dim must be > 0")
        if dim % 2 != 0:
    ...
    def forward(self, x: torch.Tensor, positions: torch.Tensor | None = None) -> torch.Tensor:
        """
        x: (B, T, dim)
        positions: (T,) もしくは (B, T) を想定。None なら [0..T-1]。
        """
        if x.dim() != 3:
            raise ValueError(f"RotaryEmbedding expects (B,T,D), got {tuple(x.shape)}")
        B, T, D = x.shape
        if D != self.dim:
            raise ValueError(f"RoPE last-dim mismatch: expected {self.dim}, got {D}")

        device = x.device
        # positions を float32 で作って、角度計算だけ float32 でやる（安定）
        if positions is None:
            pos = torch.arange(T, device=device, dtype=torch.float32)  # (T,)
        else:
            pos = positions.to(device=device, dtype=torch.float32)
            # (B,T) が来た場合も想定できるが、ここでは (T,) を主に想定
            # (B,T) の場合は後で broadcast が効くように形を整える
            if pos.dim() == 2:
                # (B,T) -> (B,T,1)
                pos = pos.unsqueeze(-1)

        inv = self.inv_freq.to(device=device, dtype=torch.float32)  # (dim/2,)

        # freqs: (T, dim/2) もしくは (B,T,dim/2)
        if pos.dim() == 1:
            freqs = pos[:, None] * inv[None, :]
            cos = freqs.cos().to(dtype=x.dtype).unsqueeze(0)  # (1,T,dim/2)
            sin = freqs.sin().to(dtype=x.dtype).unsqueeze(0)
        else:
            # pos: (B,T,1)
            freqs = pos * inv.view(1, 1, -1)
            cos = freqs.cos().to(dtype=x.dtype)  # (B,T,dim/2)
...
class NGRC_LM(nn.Module):
    ...
    def __init__(
        ...
        cross_mode : str = "projected_full",
        cross_proj_dim : int = 64,
        cross_offset : int = 0,
        ...
        if self.cross_mode == "none":
            # no buffers needed
            self.register_buffer("cross_i", torch.empty(0, dtype=torch.long), persistent=False)
            self.register_buffer("cross_j", torch.empty(0, dtype=torch.long), persistent=False)
            self.cross_dim = 0

        elif self.cross_mode == "random":
            # your original behavior (random pairs)
            if self.max_cross_terms <= 0:
                raise ValueError("random cross_mode requires max_cross_terms > 0")
            ...
            self.register_buffer("cross_i", i_idx.to(dtype=torch.long), persistent=False)
            self.register_buffer("cross_j", j_idx.to(dtype=torch.long), persistent=False)
            self.cross_dim = self.max_cross_terms

        elif self.cross_mode == "full":
            # EXACT NGRC quadratic monomials (unique upper-triangular of z⊗z)
            # WARNING: base_dimが大きいとほぼ確実に破綻します
            tri = torch.triu_indices(base_dim, base_dim, offset=int(self.cross_offset), device="cpu")
            self.register_buffer("cross_i", tri[0].to(dtype=torch.long), persistent=False)
            self.register_buffer("cross_j", tri[1].to(dtype=torch.long), persistent=False)
            self.cross_dim = tri.shape[1]

        elif self.cross_mode == "projected_full":
            # practical: project z -> m, then all i<=j products in that m-space
            if self.cross_proj_dim <= 0:
                raise ValueError("projected_full requires cross_proj_dim > 0")
            self.cross_proj = nn.Linear(base_dim, self.cross_proj_dim, bias=False)

            tri = torch.triu_indices(
                self.cross_proj_dim, self.cross_proj_dim,
                offset=int(self.cross_offset),
                device="cpu"
            )
            self.register_buffer("cross_i", tri[0].to(dtype=torch.long), persistent=False)
            self.register_buffer("cross_j", tri[1].to(dtype=torch.long), persistent=False)
        ...
        self.readout_proj = nn.Linear(self.phi_dim, readout_rank, bias=False)
        ...
        print(
            ...
            f"cross_mode={self.cross_mode}, cross_proj_dim={cross_proj_dim}, cross_offset={cross_offset}, "
            f"phi_dim={self.phi_dim}, readout_rank={readout_rank}, "
            f"use_rope={self.use_rope}, rope_dim={self.rope_dim}, rope_base={rope_base}, rope_fraction={rope_fraction}, "
    ...
    def _build_phi(self, z_flat: torch.Tensor) -> torch.Tensor:
        ...
        if self.cross_dim >0 :
            if self.cross_proj is None:
                zc = z_flat
            else:
                zc = self.cross_proj(z_flat)
            cross = zc[..., self.cross_i] * zc[..., self.cross_j]
            feats.append(cross)
...
def parse_args_ngrc():
    ...
    parser.add_argument("--ngrc_cross_mode", type=str, default = "projected_full",choices=["none", "random", "full", "projected_full"],help="クロス項のモード, none: no cross terms, random: random (i, j) pairs, full: ALL i<j products on base_dim(toy only), projected_full: project z to cross_proj_dim the ALL i<j products")
    parser.add_argument("--ngrc_cross_proj_dim", type=int, default=64, help="クロス項の投影次元")
    parser.add_argument("--ngrc_max_cross_terms", type=int, default=256, help="クロス項の最大数")
    parser.add_argument("--ngrc_cross_offset", type=int, default=0, help="クロス項のオフセット")
    parser.add_argument("--ngrc_readout_rank", type=int, default=512, help="低ランク readout 次元")
    # RoPE
    parser.add_argument("--ngrc_disable_rope", action="store_true", help="Disable RoPE on embeddings.")
    parser.add_argument("--ngrc_rope_base", type=float, default=10000.0, help="RoPE base (theta).")
    parser.add_argument("--ngrc_rope_fraction", type=float, default=1.0, help="Fraction of d_model to apply RoPE (<=1.0).")
    parser.add_argument(
        "--ngrc_embed_frozen",
        action="store_true",
...
def NGRC_experiment(lr):
    ...
    model = NGRC_LM(
        ...
        cross_mode=args.ngrc_cross_mode,
        cross_proj_dim=args.ngrc_cross_proj_dim,
        cross_offset=args.ngrc_cross_offset,
