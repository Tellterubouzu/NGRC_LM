#!/usr/bin/env python3
"""
train_gpt2_small_MLA.py
=======================
GPT‑2 small with RoPE, Multi Head Latent Attention, Flash/ME-Attention kernels
(2025/05/27)
...
import math, time, datetime, json,  os
...
# ------------------------------------------------------------------------------------------------------------------
# Enable Flash / ME‑Attention kernels globally (PyTorch ≥2.2)
# ------------------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------------------------
# Constants for uint16 shard format
# ------------------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------------------------
# Data‑loader for pre‑tokenised uint16 shards (zero‑copy mmap)
# ------------------------------------------------------------------------------------------------------------------
class BinShardsDataset(IterableDataset):
...
# ------------------------------------------------------------------------------------------------------------------
# Rotary Positional Embedding (with cache)
# ------------------------------------------------------------------------------------------------------------------
class RotaryEmbedding(nn.Module):
    def __init__(self, dim: int, base: int = 10000, scaling: float = 1.0):
        super().__init__()
    ...
    def _build_cache(self, seq_len: int, device: torch.device):
        t = torch.arange(seq_len, device=device) / self.scaling
    ...
    def get_cos_sin(self, seq_len: int, device: torch.device):
        if seq_len not in self._cache or self._cache[seq_len][0].device != device:
            return self._build_cache(seq_len, device)
    ...
    @classmethod
    def apply_rotary(cls, q: torch.Tensor, k: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor):
        return (q * cos) + (cls._rotate_half(q) * sin), (k * cos) + (cls._rotate_half(k) * sin)

# ------------------------------------------------------------------------------------------------------------------
# GPT‑like config & blocks
# ------------------------------------------------------------------------------------------------------------------
class GPTConfig:
...
class CausalSelfAttention(nn.Module):
    def __init__(self, cfg: GPTConfig, use_rope=False, mla_ratio=0.0):
        super().__init__()
        self.n_head   = cfg.n_head
        self.head_dim = cfg.n_embd // cfg.n_head
        self.dim      = cfg.n_embd
        self.use_rope = use_rope
        self.mla      = mla_ratio > 0.0
        self.latent   = max(1, int(round(self.head_dim * mla_ratio))) if self.mla else None

        self.q_proj = nn.Linear(self.dim, self.dim, bias=False)
        if self.mla:
            self.k_down, self.v_down = nn.Linear(self.dim, self.latent, False), nn.Linear(self.dim, self.latent, False)
            self.k_up,   self.v_up   = nn.Linear(self.latent, self.dim, False), nn.Linear(self.latent, self.dim, False)
        else:
            self.k_proj, self.v_proj = nn.Linear(self.dim, self.dim, False), nn.Linear(self.dim, self.dim, False)

        self.out_proj   = nn.Linear(self.dim, self.dim, False)
        self.resid_drop = nn.Dropout(cfg.resid_pdrop)
        if self.use_rope:
            self.rope = RotaryEmbedding(self.head_dim)

    def _reshape(self, x, B, L):
    ...
    def forward(self, x):
        ...
        q, k, v = (self._reshape(t, B, L) for t in (q, k, v))
        if self.use_rope:
            cos, sin = self.rope.cos_sin(L, x.device)
            q, k = RotaryEmbedding.apply(q, k, cos, sin)

        out = F.scaled_dot_product_attention(q, k, v, None, True)
        out = out.transpose(1, 2).contiguous().view(B, L, self.dim)
        return self.out_proj(self.resid_drop(out))

class GPTBlock(nn.Module):
    def __init__(self, cfg: GPTConfig, use_rope=False, mla_ratio=0.0):
        super().__init__()
        self.ln1 = nn.LayerNorm(cfg.n_embd, eps=cfg.layer_norm_epsilon)
        self.attn = CausalSelfAttention(cfg, use_rope, mla_ratio)
...
class GPTLMHeadModel(nn.Module):
    def __init__(self, cfg: GPTConfig, use_rope=False, mla_ratio=0.0):
        ...
        self.h    = nn.ModuleList([GPTBlock(cfg, use_rope, mla_ratio) for _ in range(cfg.n_layer)])
        self.ln_f = nn.LayerNorm(cfg.n_embd, eps=cfg.layer_norm_epsilon)
        self.lm_head = nn.Linear(cfg.n_embd, cfg.vocab_size, False)
...
# ------------------------------------------------------------------------------------------------------------------
# Helper utilities (API keys, Dataset, etc.) 
# ------------------------------------------------------------------------------------------------------------------
...
def parse_args():
    ...
    parser.add_argument("--use_rope",action = "store_true", help = "Use RoPE")
    parser.add_argument("--mla_rank_ratio", type = float, default = 4, help = "r / head_dim for Multi-Head Latent Attention (0 → disable MLA)")
...
def main():
    ...
    model = GPTLMHeadModel(
        ...
        mla_rank_ratio=args.mla_rank_ratio
        ).to(torch.bfloat16)
    ...
    for step, batch in enumerate(train_loader, start=1):
        ...
        tokens_seen_global += args.local_batch_size * args.seq_len * world
