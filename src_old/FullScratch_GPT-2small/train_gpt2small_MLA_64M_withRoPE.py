...
class CausalSelfAttention(nn.Module):
    def __init__(self, cfg: GPTConfig, use_rope=False, mla_rank_ratio=0.0):
        super().__init__()
        self.n_head   = cfg.n_head
        self.head_dim = cfg.n_embd // cfg.n_head
        self.dim      = cfg.n_embd
        self.use_rope = use_rope
        self.mla      = mla_rank_ratio > 0.0
        self.latent   = max(1, int(round(self.head_dim * mla_rank_ratio))) if self.mla else None
...
class GPTBlock(nn.Module):
    def __init__(self, cfg: GPTConfig, use_rope=False, mla_rank_ratio=0.0):
        ...
        self.attn = CausalSelfAttention(cfg, use_rope, mla_rank_ratio)
...
class GPTLMHeadModel(nn.Module):
    def __init__(self, cfg: GPTConfig, use_rope=False, mla_rank_ratio=0.0):
        ...
        self.h    = nn.ModuleList([GPTBlock(cfg, use_rope, mla_rank_ratio) for _ in range(cfg.n_layer)])
...
def parse_args():
    ...
    parser.add_argument("--local_batch_size", type=int, default=32, help="Micro batch size per GPU")
    ...
    # Model parameters
    parser.add_argument("--n_embd", type=int, default=512, help="Embedding dimension size")
...
def main():
    ...
    if not distributed or args.local_rank == 0:
            wandb_run_name = args.wandb_run_name or f"GPT(MLA){param_millions:.2f}M_withRoPE_batch_size{args.local_batch_size}_seq_len{args.seq_len}_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
