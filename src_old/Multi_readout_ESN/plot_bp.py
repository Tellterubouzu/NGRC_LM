...
esn_mro_df = pd.read_csv('/home/ach17664ll/GPTvsESN/src/Multi_readout_ESN/run_table_metrics.csv')


# 欠損値を削除してから配列に変換
esn_val_perplexity_values = esn_df['final_val_perplexity'].dropna().values
gpt_val_perplexity_values = gpt_df['final_val_perplexity'].dropna().values
esn_mro_val_perplexity_values = esn_mro_df['final_val_perplexity'].dropna().values

esn_mro_df['final_val_perplexity'] = pd.to_numeric(
    esn_mro_df['final_val_perplexity'], errors='coerce'
)
print(esn_mro_df['final_val_perplexity'].describe())
print(esn_mro_df['final_val_perplexity'].head(10))
print(esn_mro_df.columns)

print(esn_mro_df.head())
...
plt.text(0.5, 0.98, esn_mro_stats_text, transform=plt.gca().transAxes, 
         verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
plt.text(0.8, 0.98, gpt_stats_text, transform=plt.gca().transAxes, 
         verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
# レイアウトを調整
plt.tight_layout()

# 画像を保存
plt.savefig('val_perplexity_boxplot.png', dpi=300, bbox_inches='tight')
