...
class ESN_mlr(nn.Module):
    ...
    def __init__(
        ...
        # ---------- sparse W_rec (N × N) ----------
        W_rec = self._rand_sparse((self.reservoir_size, self.reservoir_size), self.gamma,self.sigma_rec, device)
...
def parse_args_esn():
    ...
    parser.add_argument("--sigma_rec", type=float, default=1.0, help="Recurrent weight init scale")
    ...
    # Weights & Biases
    parser.add_argument("--wandb_project", type=str, default="ESN Lanugage Model", help="WandB project name")
...
def ESN_experiment(lr, sigma_rec):
    # =============================================================================
    ...
    for step, batch in enumerate(train_loader, start=1):
        ...
        if step % args.save_checkpoint_every_steps == 0 and args.local_rank==0:
            ...
            save_dir = f"./checkpoint/ESN_ml ({param_millions:.2f}M N{args.reservoir_size}_batch_size{args.local_batch_size}_seq_len{args.seq_len}_sigma_in{args.sigma_in}_spectral_radius{args.spectral_radius}_sparsity{1-(args.d/args.reservoir_size)}_dropout{args.dropout}_r_out{args.r_out}_{id}"
...
if __name__ == "__main__":
    for sigma_rec in [0.5,0.75,0.25,0.01]:
        for i in range(5):

            try:
                lr = 1e-3
                print(f"lr: {lr}")
                ESN_experiment(lr, sigma_rec)
