...
def main():
    ...
    if not distributed or args.local_rank == 0:
            wandb_run_name = args.wandb_run_name or f"ESN_({param_millions:.2f}M N{args.reservoir_size}_leakrate{args.leaking_rate}_sigma_in{args.sigma_in}_spectral_radius{args.spectral_radius}_sparsity{args.reservoir_sparsity}_dropout{args.dropout}_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    if is_master:
        ...
        if WANDB_AVAILABLE:
            ...
            for k in report_ks:
                ...
                table.add_data(k,mean_so_far[k])
