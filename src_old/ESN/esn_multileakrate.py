...
def prepare_hf_repo_id(write_token, explicit_repo, a_min,a_max,default_prefix="ESN-scratch-1024-lr"):
    api = HfApi(token=write_token)
    if explicit_repo:
        return explicit_repo, api
    owner = api.whoami()["name"]
    timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    repo_id = f"{owner}/{default_prefix}-{a_min}-{a_max}-{timestamp}"
    return repo_id, api
...
class ESN_mlr(nn.Module):
    ...
    def __init__(
        ...
        print(f"ESN_mlr initialized with \n {self.vocab_size} vocab size,\n {self.reservoir_size} \nreservoir size, \n{self.d} sparsity,\n {self.gamma} gamma,\n {self.sigma_in} sigma_in, \n{self.alpha_min} alpha_min, \n{self.alpha_max} alpha_max, \n{self.activation} activation, \n{self.dropout} dropout, \n{self.r_out} r_out")
        try:
            import wandb
...
def parse_args():
    ...
    # ESN
    parser.add_argument("--reservoir_size", type=int, default=65536, help="ESN reservoir size (N_rec)")
    ...
    # Weights & Biases
    parser.add_argument("--wandb_project", type=str, default="ESN Lanugage Model leak rate experiment", help="WandB project name")
...
def mains():
    ...
    if not distributed or args.local_rank == 0:
            wandb_run_name = args.wandb_run_name or f"ESN_ml ({param_millions:.2f}M N{args.reservoir_size}_batch_size{args.local_batch_size}_seq_len{args.seq_len}_sigma_in{args.sigma_in}_spectral_radius{args.spectral_radius}_sparsity{1-(args.d/args.reservoir_size)}_dropout{args.dropout}_r_out{args.r_out}_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    if is_master:
        ...
        if "HF_WRITE_TOKEN" in api_keys:
            ...
            api.create_repo(repo_id=repo_id, exist_ok=True, a_min=args.alpha_min,a_max=args.alpha_max,private=args.hf_private)
...
if __name__ == "__main__":
    ...
    for (mi, ma) in combos:
        ...
        sys.argv = [
            ...
            "--alpha_min", str(mi),
            "--alpha_max", str(ma),
            "--total_tokens", str(min(base_args.total_tokens, 1e7)),  # 予算圧縮
