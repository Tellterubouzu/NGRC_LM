...
class ESN_mlr(nn.Module):
    ...
    @staticmethod
    @torch.no_grad()
    def _scale_spectral_radius(mat, target_rho, iters=20):
...
def parse_args():
    ...
    parser.add_argument("--local_batch_size", type=int, default=16, help="Micro batch size per GPU")
...
def main():
    ...
    for step, batch in enumerate(train_loader, start=1):
        ...
        if tokens_seen_global >= args.total_tokens:
            break
    if FORCE_STOP:
        print("⚠️ Training stopped due to NaN loss")
    else:
